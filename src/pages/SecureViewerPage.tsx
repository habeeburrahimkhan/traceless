...
const TiltCard: React.FC<TiltCardProps> = ({ children, className = '', glowColor = 'rgba(6, 182, 212, 0.25)' }) => {
...
export const SecureViewerPage: React.FC = () => {
  ...
  if (activeViewerDocId && !doc && isLoading) {
    return (
      <div className="max-w-md mx-auto py-12 px-4 text-center text-zinc-400 font-mono text-sm animate-pulse">
        Loading secure document envelope...
      </div>
    );
  }

  // RENDER LEVEL 1: Standby Entry Card
  if (!doc) {
    const activeDocs = isAdmin ? documents.filter((d) => d.status === 'active') : [];

    return (
      <div className="max-w-5xl mx-auto py-12 px-4 animate-fade-in text-center">
        <style>{`
          @keyframes spinDecoderGyro {
            0% { transform: rotateX(60deg) rotateZ(0deg); }
            100% { transform: rotateX(60deg) rotateZ(-360deg); }
          }
          @keyframes spinDecoderShield {
            0% { transform: rotateY(0deg) rotateX(-20deg); }
            100% { transform: rotateY(-360deg) rotateX(-20deg); }
          }
          .decoder-3d-rings {
            transform-style: preserve-3d;
            animation: spinDecoderGyro 16s linear infinite;
          }
          .decoder-3d-shield {
            transform-style: preserve-3d;
            animation: spinDecoderShield 12s linear infinite;
          }
        `}</style>
        
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          {/* Left/Center side: original standby card (col-span-8) */}
          <div className="lg:col-span-8 flex flex-col justify-start">
            <TiltCard className="rounded-2xl border border-zinc-800 bg-zinc-955/95 shadow-[0_4px_30px_rgba(0,0,0,0.5)]" glowColor="rgba(6, 182, 212, 0.2)">
              <div className="p-6 sm:p-8 space-y-6 relative overflow-hidden text-left">
                ...
                <form onSubmit={handleDocIdSubmit} className="space-y-4 font-mono">
                  <div className="space-y-2">
                    ...
                    <div className="flex bg-zinc-900 border border-zinc-855 rounded-lg p-2.5 items-center gap-3 focus-within:border-cyan-500/50 transition-colors">
                  ...
                  <button
                    type="submit"
                    className="w-full bg-cyan-500 hover:bg-cyan-400 text-black py-2.5 rounded-lg text-xs font-semibold font-mono tracking-wider uppercase transition-colors shadow-[0_4px_15px_rgba(6,182,212,0.15)]"
                  >
                ...
                {isAdmin && (
                  <div className="pt-4 border-t border-zinc-900 space-y-3 font-mono text-[10px]">
                    ...
                    <div className="flex flex-col gap-1.5" onClick={(e) => e.stopPropagation()}>
                      {activeDocs.length === 0 ? (
                        <span className="text-zinc-655 italic">No active shares found. Upload a document first.</span>
                      ) : (
                        activeDocs.map((d) => (
                          <button
                              key={d.id}
                              ...
                              className="flex justify-between items-center bg-zinc-900/40 border border-zinc-850 hover:border-cyan-500/20 px-2.5 py-1.5 rounded text-left text-zinc-400 hover:text-zinc-200 transition-colors z-20 relative"
                            >
          ...
          {/* Right side: 3D Decoder Gyro Node & Shield Core (col-span-4) */}
          <div className="hidden lg:flex lg:col-span-4 flex-col items-center justify-between space-y-4 border border-zinc-850 bg-zinc-955/95 p-6 rounded-2xl relative overflow-hidden min-h-[500px]">
            ...
            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">DECRYPT DECODER NODE</span>
            
            {/* Object 1: Decoder Gyro rings */}
            <div className="w-40 h-40 relative flex items-center justify-center" style={{ perspective: '800px' }}>
              <div className="w-full h-full relative decoder-3d-rings">
                <div className="absolute inset-0 border-4 border-dashed border-cyan-500 rounded-full opacity-70" style={{ filter: 'drop-shadow(0 0 10px rgba(6,182,212,0.4))' }} />
                <div className="absolute inset-4 border-2 border-double border-emerald-500 rounded-full opacity-75" style={{ transform: 'rotateY(90deg)', filter: 'drop-shadow(0 0 8px rgba(16,185,129,0.4))' }} />
                <div className="absolute inset-8 border border-zinc-800 rounded-full" style={{ transform: 'rotateX(90deg)' }} />
              </div>
              <div className="absolute inset-0 flex items-center justify-center">
                <Key className="h-9 w-9 text-cyan-400 animate-pulse" />
              </div>
            </div>

            {/* Object 2: Decoder Shield Core */}
            <div className="w-24 h-24 relative flex items-center justify-center border-t border-zinc-900/60 pt-4 w-full animate-fade-in" style={{ perspective: '800px' }}>
              <div className="w-18 h-18 relative decoder-3d-shield">
                <div className="absolute inset-0 border border-double border-emerald-500 rounded-full opacity-60" style={{ filter: 'drop-shadow(0 0 6px rgba(16,185,129,0.3))' }} />
                <div className="absolute inset-1.5 border border-dashed border-cyan-400 rounded-full opacity-50" style={{ transform: 'rotateY(90deg)' }} />
                <div className="absolute inset-3 border border-zinc-700 rounded-full" style={{ transform: 'rotateX(90deg)' }} />
                <div className="absolute inset-4.5 border border-cyan-500 rounded-full flex items-center justify-center">
            ...
            <div className="space-y-1.5 font-mono text-[10px] text-zinc-500 text-center w-full">
              <p>SANDBOX: <span className="text-cyan-400 font-bold">STANDBY</span></p>
              <p>IP LOCKING: <span className="text-zinc-300 font-bold">ACTIVE</span></p>
              <p className="text-[9px] text-zinc-650">Ready to ingest OTP Decryption stream.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  // RENDER LEVEL 2: Verification gateway cards wrapped in TiltCard
  if (doc.status !== 'active' || !viewerAuthenticated) {
    return (
      <div className="max-w-5xl mx-auto py-12 px-4 animate-fade-in text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          {/* Left/Center side: original verification gateway card (col-span-8) */}
          <div className="lg:col-span-8 flex flex-col justify-start">
            <TiltCard className="rounded-2xl border border-zinc-800 bg-zinc-955/95 shadow-[0_4px_30px_rgba(0,0,0,0.5)]" glowColor={verifyStage === 'failure' ? 'rgba(244, 63, 94, 0.25)' : 'rgba(16, 185, 129, 0.25)'}>
              <div className="p-6 sm:p-8 space-y-6 relative overflow-hidden text-left">
                ...
                <div className="flex justify-between items-center font-mono">
                  <button 
                    ...
                    className="text-[10px] text-zinc-500 hover:text-zinc-300 flex items-center gap-1 z-20 relative"
                  >
                    &larr; Switch File
                  </button>
                  ...
                  isPrintDone ? (
                    <div className="space-y-6 text-center pt-2 font-mono animate-fade-in">
                      ...
                      <button
                        ...
                        className="w-full bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-zinc-300 py-2.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors z-20 relative"
                      >
                        Back to Terminal
                      </button>
                    </div>
                    ...
                    <div className="space-y-6 text-center pt-2 font-mono">
                      <div className="h-14 w-14 rounded-full bg-rose-500/10 border-2 border-rose-500 flex items-center justify-center mx-auto">
                      ...
                      <button
                        onClick={() => navigate('viewer', null)}
                        className="w-full bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-zinc-300 py-2.5 rounded-lg text-xs font-semibold z-20 relative"
                      >
                        Back to Terminal
                      </button>
                    </div>
                  ...
                  <>
                    {verifyStage === 'email' && (
                      <form onSubmit={handleEmailSubmit} className="space-y-5 font-mono">
                        ...
                        <button
                          type="submit"
                          className="w-full bg-emerald-500 hover:bg-emerald-400 text-black py-2.5 rounded-lg text-xs font-semibold tracking-wider uppercase transition-colors shadow-[0_4px_15px_rgba(16,185,129,0.15)]"
                        >
                    ...
                    {verifyStage === 'otp' && (
                      <form onSubmit={handleOTPVerify} className="space-y-5 font-mono">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex bg-zinc-900 border border-zinc-850 rounded-lg p-2.5 items-center gap-3">
                            ...
                            <input
                              ...
                              placeholder="000000"
                              className="bg-transparent border-none text-xs text-zinc-200 tracking-widest font-bold focus:outline-none w-full text-center font-mono"
                            />
                          </div>
                        </div>
                        ...
                        {doc.requireEmailVerification ? (
                          <div className="flex justify-between items-center text-[10px] pt-1 text-zinc-500" onClick={(e) => e.stopPropagation()}>
                            <button 
                              type="button" 
                              onClick={() => setVerifyStage('email')} 
                              className="hover:text-zinc-300 hover:underline z-20 relative"
                            >
                              &larr; Adjust Email
                            </button>
                            <button 
                          ...
                          <div className="flex justify-start text-[10px] pt-1 text-zinc-500" onClick={(e) => e.stopPropagation()}>
                            <button 
                              type="button" 
                              onClick={() => navigate('viewer', null)} 
                              className="hover:text-zinc-300 hover:underline z-20 relative"
                      ...
                      <div className="py-6 text-center space-y-6 font-mono animate-fade-in">
                        ...
                        <div className="flex gap-2 pt-2" onClick={(e) => e.stopPropagation()}>
                          <button
                            ...
                            className="flex-1 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-300 py-2.5 rounded-lg text-xs font-semibold z-20 relative"
                          >
                            Retry Verification
                          </button>
                          <button
                            type="button"
                            onClick={() => { navigate('viewer', null); }}
                            className="flex-1 bg-rose-950/20 hover:bg-rose-500 hover:text-black border border-rose-900/30 text-rose-400 py-2.5 rounded-lg text-xs font-semibold transition-all z-20 relative"
                          >
                            Abort Session
                          </button>
                        </div>
  ...
  // RENDER LEVEL 3: Restricted Admin Boundary
  if (isAdmin && viewerAuthenticated) {
    return (
      <div className="max-w-5xl mx-auto py-12 px-4 animate-fade-in text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          {/* Left/Center side: admin shield card (col-span-8) */}
          <div className="lg:col-span-8 flex flex-col justify-start">
            <TiltCard className="rounded-2xl border border-rose-500/40 bg-zinc-955/95 shadow-[0_4px_30px_rgba(0,0,0,0.5)]" glowColor="rgba(244, 63, 94, 0.25)">
              <div className="p-6 sm:p-8 space-y-6 relative overflow-hidden text-left">
                ...
                <div className="flex justify-between items-center font-mono">
                  ...
                  <span className="text-[9px] bg-rose-955/40 border border-rose-900/40 text-rose-400 px-2 py-0.5 rounded font-mono font-bold uppercase">
                    RESTRICTED
                  </span>
                </div>
                ...
                <div className="space-y-4 pt-2 text-center font-mono">
                  ...
                  <div className="flex flex-col gap-2 pt-2" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      className="w-full bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 hover:text-white text-zinc-300 py-2.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all z-20 relative"
                    >
                      Return to Dashboard
                    </button>
                  </div>
  ...
  // RENDER LEVEL 4: Decrypted Document Preview Sandbox (Non-tilted to prevent layout distortion)
  return (
